...
func (m *temperaturePanel) initialize() {
	...
	m.Grid().Attach(m.createChangeButton(ui_lang.Translate("Increase"), "increase.svg", 1), 1, 0, 1, 1)
	m.Grid().Attach(m.createChangeButton(ui_lang.Translate("Decrease"), "decrease.svg", -1), 5, 0, 1, 1)
	...
	m.Grid().Attach(MustButtonImage(ui_lang.Translate("Profiles"), "heat-up.svg", m.profilePanel), 3, 1, 1, 1)
}
...
func (m *temperaturePanel) increaseTarget(tool string, value float64) error {
	...
	Logger.Infof(ui_lang.Translate("Setting target temperature for %s to %1.f°C."), ui_lang.Translate(tool), target)
...
func (m *temperaturePanel) getToolTarget(tool string) (float64, error) {
	...
	if !ok {
		return -1, fmt.Errorf(ui_lang.Translate("unable to find tool %q"), ui_lang.Translate(tool))
	}
...
func (m *profilesPanel) loadProfiles() {
	...
	m.AddButton(m.createProfileButton("cool-down.svg", &octoprint.TemperatureProfile{
		Name:     ui_lang.Translate("Cool Down"),
		Bed:      0,
		Extruder: 0,
	}))
}
...
func (m *profilesPanel) createProfileButton(img string, p *octoprint.TemperatureProfile) gtk.IWidget {
	return MustButtonImage(p.Name, img, func() {
		Logger.Warningf(ui_lang.Translate("Setting temperature profile %s."), p.Name)
